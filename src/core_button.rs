...
/// An event which is emitted when a button is clicked. This is different from the `Click` event,
/// because it's also emitted when the button is focused and the Enter or Space key is pressed.
#[derive(Clone, Debug)]
